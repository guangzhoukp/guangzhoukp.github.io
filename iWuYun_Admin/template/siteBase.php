...
<?php include tpl('menu');?>
	<div class="main">
		...
		<?php
			w('text')->set('name','需要伪造的网址')
					->set('key','host')
					->set('value',d('config')->get('host'))
					->set('tipe','整站需要伪造的网址，如:http://www.baidu.com/')
					->e();
			...
			w('select')->set('name','替换HTML相对地址')
					...
					->set('options',array('替换'=>'0','不替换'=>'1'))
					->set('tipe','替换相对地址，可以让在二级目录的乌云CMS正常运行，影响样式文件、脚本、站内链接')
					->e();
			w('select')->set('name','替换CSS相对地址')
					->set('key','relativeCSS')
					->set('value',d('config')->get('relativeCSS'))
					->set('options',array('替换'=>'0','不替换'=>'1'))
					->set('tipe','替换相对地址，可以让在二级目录的乌云CMS正常运行，影响样式文件中的图片')
					->e();
		?>
		<br>
